...
void CadastroADM(Evento E[], int tam, char C[][3]){
    ...
    cin>>Pos;
    if(Pos >= 0 && Pos<tam && E[Pos].Descricao!="0"){ // Fiz uma condição pra garantir que o evento esteja no intervalo certo. Sem ser negativo ou maior do que o tam.
...
bool AutenticarADM(){ // Adicionei a opcao do ADM, só quando a pessoa selecionar esse acesso e digitar o código correto, aparece o MENU separado. - Murilo
    ...
    cin>>Codigo;
    return (Codigo=="ADMIN2"); // Se digitar exatamente isso, retorna 1 (verdadeiro). Se não, 0 (falso). - Murilo
}
